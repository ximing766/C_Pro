...
void process_macdata(uint8_t key[], uint8_t macdata[], int length, uint8_t iv[],uint8_t des[8]) {
    ...
    for (i = 0; i < length / 8; i++) {
        uint8_t part[8];
        for (j = 0; j < 8; j++) {
            part[j] = macdata[i * 8 + j];
        }
        if(i==0)
        {
            xor_arrays(part,iv,part,8);
            (void)des3_ecb_encrypt(des,part, 8, key, 16);
        }else {
            xor_arrays(des,part,des,8);
            (void)des3_ecb_encrypt(des,des, 8, key, 16);
        }
    }
}

void removeFirstChar(char cardNoStr[]) {
    size_t len = strlen(cardNoStr);
    if (len > 0) {
        memmove(cardNoStr, cardNoStr + 1, len - 1);
        cardNoStr[len - 1] = '\0'; // 添加字符串结束符
    }
}
...
void hexArrayToString(unsigned char *hexArray, char *result, int length) {
    int i;
    for (i = 0; i < length; i++) {
        unsigned char high = hexArray[i] >> 4;
        unsigned char low = hexArray[i] & 0x0F; 

        result[i*2] = high < 10 ? '0' + high : 'A' + (high - 10);
        result[i*2 + 1] = low < 10 ? '0' + low : 'A' + (low - 10);
    }
    result[i*2] = '\0';
}
...
void StringToHex(char *factor, uint8_t *factor_hex) {
	int i;
    for (i = 0; i < strlen(factor); i += 2) {
        char hex[3];
        hex[0] = factor[i];
        hex[1] = factor[i + 1];
        hex[2] = '\0';
        uint8_t dec = (uint8_t)strtol(hex, NULL, 16);
        factor_hex[i / 2] = dec;
    }
}
...
void appendArray(uint8_t src[], uint8_t dest[], size_t len) {
    size_t dest_len = strlen((char*)dest);
    size_t i;
    for (i = 0; i < len; i++) {
...
void main(void)
{
    uint8_t defaultkey[]     = {0x32, 0xD4, 0x64, 0xAC, 0x81, 0xF1, 0x64, 0x0A, 0x68, 0x7D, 0x02, 0x3B, 0xF9, 0x9E, 0x35, 0xDF};
    uint8_t posID[] = {0x04, 0x09, 0x00, 0x01, 0x00, 0x01};
    uint8_t tradeMoney[] = {0x00, 0x00, 0x00, 0x01};
    uint8_t cardnumber[10] = {0x03, 0x10, 0x48, 0x70, 0x49, 0x41, 0x20, 0x26, 0x42, 0x55};
    uint8_t cardNoStr[20];
    uint8_t factor[20] ;
    uint8_t factor_mask[8] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
    uint8_t factor_hex[16] = {0};
    uint8_t factor_hex_temp[8];

    uint8_t requestMessage[] = {0x80, 0x50, 0x03, 0x02, 0x0B, 0x01, 0x00, 0x00, 0x00, 0x01, 0x04, 0x09, 0x00, 0x01, 0x00, 0x01, 0x0F};
    uint8_t randomNo[] = {0xC5,0x7D,0xB3,0x21,0x00,0x05,0x00,0x01};
    uint8_t sessionkey_3des[8];

    uint8_t macdata[] = {0x00, 0x00, 0x00, 0x01, 0x09, 0x04, 0x09, 0x00, 0x01, 0x00, 0x01, 0x20, 0x24, 0x08, 0x21, 0x18, 0x58, 0x44,0x80,0x00,0x00,0x00,0x00,0x00};
    uint8_t iv[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    uint8_t mac[8];
    uint8_t sessionkey_des[16];
    uint8_t test_sessionkey[16]={0xA4,0xAC,0xC3,0xB7,0x6F,0xA2,0xCC,0xBD,0xA4,0xAC,0xC3,0xB7,0x6F,0xA2,0xCC,0xBD};

    hexArrayToString(cardnumber, cardNoStr, 10);
    removeFirstChar(cardNoStr);
    size_t len = strlen(cardNoStr);
    int i;
    for( i = 0; i < 16; i++) {
        factor[i] = cardNoStr[len - 16 + i];
    }
    StringToHex(factor, factor_hex);
    memcpy(factor_hex_temp,factor_hex,8);
    xor_arrays(factor_hex_temp,factor_mask,factor_hex_temp,8);
    appendArray(factor_hex_temp, factor_hex, sizeof(factor_hex_temp));

    uint8_t loadKey[16];
    (void)des3_ecb_encrypt(loadKey,factor_hex, 16, defaultkey, 16);

    des3_ecb_encrypt(sessionkey_3des,randomNo, 8, loadKey, 16);
    memcpy(sessionkey_des, sessionkey_3des, 8);
    memcpy(sessionkey_des + 8, sessionkey_3des, 8);
    
    for(int i = 0; i < 16; i++)
    {
        printf("%02x",sessionkey_des[i]);
    }
    printf("\nsessionkey_des = \n");
    int length = sizeof(macdata) / sizeof(macdata[0]);
    process_macdata(sessionkey_des, macdata, length, iv, mac);   //最终mac仅要前4字节
    for(int i=0;i<8;i++)
    {
        printf("%x",mac[i]);
    }
    ...
    // printf("\n");
    // int decrpt_result = des3_ecb_decrypt(dencrypt_buf, encrypt_buf, encrypt_buf_len, loadkey, 16);
    // if(decrpt_result == 0)
    // {
    //     for(int i = 0;i < 8;i++)
    //     {
    //         printf("%x",dencrypt_buf[i]);
    //     }
    // }
